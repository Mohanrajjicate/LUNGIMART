...
const LoginPage: React.FC<{ onLogin: () => void }> = ({ onLogin }) => {
  return (
    <div className="w-full max-w-sm mx-auto bg-white p-8 rounded-lg shadow-md">
        <h2 className="text-2xl font-bold text-center text-dark mb-2">Welcome Back!</h2>
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); onLogin(); }}>
            ...
            <button type="submit" className="w-full bg-primary text-white font-bold py-2.5 px-4 rounded-full hover:bg-primary-dark transition-colors">
                Log In
            </button>
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'orders':
        return (
          <div className="space-y-4">
            <h2 className="text-xl font-bold text-dark mb-4">Order History</h2>
      ...
      case 'wishlist':
        return (
          <div>
            <h2 className="text-xl font-bold text-dark mb-4">Wishlist ({wishlist.length})</h2>
            {wishlist.length > 0 ? (
                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {wishlist.map(product => <ProductCard key={product.id} product={product} />)}
                </div>
            ) : (
                <div className="bg-white text-center py-16 rounded-lg border border-gray-200">
      ...
      case 'address':
        return (
            <div className="bg-white p-6 rounded-lg border border-gray-200 max-w-md">
               <h3 className="text-xl font-bold text-dark mb-4">Address Book</h3>
  ...
  return (
    <div>
        <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-8 gap-4">
            ...
            <button onClick={logout} className="bg-dark text-white font-semibold py-2 px-6 rounded-full hover:bg-dark-light transition-colors self-start sm:self-center">
                Logout
            </button>
        </div>
        ...
        <div className="md:grid md:grid-cols-12 md:gap-8">
            ...
            <div className="hidden md:block md:col-span-3 lg:col-span-2">
                 <nav className="flex flex-col space-y-2" aria-label="Tabs">
                    <button onClick={() => setActiveTab('orders')} className={`text-left px-4 py-2 rounded-md font-medium text-sm ${activeTab === 'orders' ? 'bg-primary/10 text-primary font-bold' : 'text-gray-600 hover:bg-light-200'}`}>Order History</button>
                    <button onClick={() => setActiveTab('wishlist')} className={`text-left px-4 py-2 rounded-md font-medium text-sm ${activeTab === 'wishlist' ? 'bg-primary/10 text-primary font-bold' : 'text-gray-600 hover:bg-light-200'}`}>Wishlist</button>
                    <button onClick={() => setActiveTab('address')} className={`text-left px-4 py-2 rounded-md font-medium text-sm ${activeTab === 'address' ? 'bg-primary/10 text-primary font-bold' : 'text-gray-600 hover:bg-light-200'}`}>Address Book</button>
                </nav>
            </div>
            ...
            <div className="md:hidden border-b border-gray-200 mb-6">
                <nav className="-mb-px flex space-x-6 overflow-x-auto" aria-label="Tabs">
                    ...
                    <button onClick={() => setActiveTab('wishlist')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'wishlist' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>Wishlist</button>
