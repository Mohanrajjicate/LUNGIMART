...
const AppearancePage: React.FC = () => {
    ...
    const [editingItemType, setEditingItemType] = useState<'banner' | 'category' | null>(null);
    ...
    const openGallery = (id: string, type: 'banner' | 'category') => {
        setEditingItemId(id);
    ...
    const handleImageSelect = (imageUrl: string) => {
        ...
        if (editingItemType === 'banner') {
            setPendingBanners(current => current.map(item => item.id === editingItemId ? { ...item, imageUrl: imageUrl } : item));
        } else {
            setPendingCategories(current => current.map(item => String(item.id) === editingItemId ? { ...item, image: imageUrl } : item));
        }
    };
                                ...
                                <div className="md:col-span-2 flex items-center gap-6">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <label
                                            ...
                                            className="cursor-pointer bg-slate-700 text-white font-bold py-2 px-4 rounded-lg hover:bg-slate-600 transition-colors text-sm text-center"
                                        >
                                            Upload New
                                        </label>
                                        <button
                                            type="button"
                                            onClick={() => openGallery(banner.id, 'banner')}
                                            className="bg-slate-100 text-slate-700 font-bold py-2 px-4 rounded-lg hover:bg-slate-200 transition-colors text-sm"
                                        >
                                            From Gallery
                                        </button>
                                    </div>
                            ...
                            <div key={category.id} className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center border-b pb-6 last:border-b-0 last:pb-0">
                                ...
                                <div className="md:col-span-2 flex items-center gap-6">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => openGallery(String(category.id), 'category')}
                                            className="bg-slate-100 text-slate-700 font-bold py-2 px-4 rounded-lg hover:bg-slate-200 transition-colors text-sm"
                                        >
                                            From Gallery
                                        </button>
