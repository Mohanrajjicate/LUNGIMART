...
const baseProducts: Omit<Product, 'reviews' | 'rating' | 'reviewCount'>[] = [
  {
    id: 1,
...
const attachReviewData = (product: Omit<Product, 'reviews' | 'rating' | 'reviewCount'>, allReviews: Review[]): Product => {
...
// Function to get all products with review data attached
export const getAllProducts = (allReviews: Review[]): Product[] => {
    return baseProducts.map(p => attachReviewData(p, allReviews));
}

export const getProductBySlug = (slug: string, allReviews: Review[]): Product | undefined => {
    const product = baseProducts.find(p => p.slug === slug);
    return product ? attachReviewData(product, allReviews) : undefined;
}

export const getProductsByCategory = (categorySlug: string, allReviews: Review[]): Product[] => {
    const allProducts = getAllProducts(allReviews);

    if (!categorySlug || categorySlug === 'all') return allProducts;

    if (categorySlug === 'best-selling') {
        return allProducts.filter(p => (p.rating || 0) >= 4.5).sort((a,b) => (b.reviewCount || 0) - (a.reviewCount || 0));
    }
    ...
    if (categorySlug === 'temple-vibe') {
        ...
        return allProducts.filter(p => templeProductIds.includes(p.id));
    }
...
export const getRelatedProducts = (currentProductId: number, allReviews: Review[]): Product[] => {
    const allProducts = getAllProducts(allReviews);
