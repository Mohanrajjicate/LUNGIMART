import { Product, Coupon, Category, Review, User, Order, Banner } from '../types';

// --- DATA (Moved from server.js) ---
...
const baseProductsRaw: Omit<Product, 'reviews' | 'rating' | 'reviewCount'>[] = [
  { id: 1, name: 'Classic Checkered Lungi', slug: 'classic-checkered-lungi', category: getCategoryBySlug('checkered-lungi'), price: 499, originalPrice: 799, images: ['https://picsum.photos/seed/product1/800/800', 'https://picsum.photos/seed/product1-2/800/800', 'https://picsum.photos/seed/product1-3/800/800'], description: 'A timeless classic, this checkered lungi is made from 100% pure Komarapalayam cotton, known for its softness and durability. Perfect for daily wear.', details: ['100% Cotton', 'Machine Washable', '2.25 meters length', 'Colorfast guarantee'], stock: 120, virtualCategories: ['best-selling', 'featured-products'] },
  { id: 2, name: 'Pure White Temple Dhoti', slug: 'pure-white-temple-dhoti', category: getCategoryBySlug('temple-dhoti'), price: 899, originalPrice: 1199, images: ['https://picsum.photos/seed/product2/800/800', 'https://picsum.photos/seed/product2-2/800/800'], description: 'Feel divine in our pure white temple dhoti. Woven with care and precision, it offers unparalleled comfort and a graceful drape for all your spiritual occasions.', details: ['Premium Weave Cotton', 'Hand Wash Recommended', '4 meters length', 'Slightly starched for a crisp look'], stock: 75 },
  { id: 3, name: 'Silk Dhoti with Angavastram', slug: 'silk-dhoti-with-angavastram', category: getCategoryBySlug('matching-dhoti'), price: 1599, originalPrice: 2199, images: ['https://picsum.photos/seed/product3/800/800', 'https://picsum.photos/seed/product3-2/800/800'], description: 'Exquisite silk-blend matching dhoti and angavastram set from the master weavers of Komarapalayam. Features a beautiful golden border for a regal touch.', details: ['Silk-Cotton Blend', 'Dry Clean Only', 'Dhoti: 4m, Angavastram: 2.5m', 'Intricate Zari Border'], stock: 40, virtualCategories: ['featured-products'] },
  { id: 4, name: 'DMK Flag Border Dhoti', slug: 'dmk-flag-border-dhoti', category: getCategoryBySlug('political-party'), price: 950, originalPrice: 1299, images: ['https://picsum.photos/seed/product4/800/800', 'https://picsum.photos/seed/product4-2/800/800'], description: 'Show your support with this premium cotton dhoti featuring the iconic red and black border. Woven for comfort and durability.', details: ['100% Cotton', 'Machine Washable', '4 meters length', 'Vibrant, colorfast border'], stock: 8 },
  { id: 5, name: 'Absorbent Bath Towel', slug: 'absorbent-bath-towel', category: getCategoryBySlug('towel'), price: 699, originalPrice: 999, images: ['https://picsum.photos/seed/product5/800/800', 'https://picsum.photos/seed/product5-2/800/800'], description: 'A large, highly absorbent bath towel made from the finest Komarapalayam cotton. Soft to the touch and quick-drying.', details: ['Premium Terry Cotton', 'Machine Washable', 'Size: 75cm x 150cm', 'High GSM for absorbency'], stock: 150, virtualCategories: ['best-selling'] },
  { id: 6, name: 'Striped Casual Lungi', slug: 'striped-casual-lungi', category: getCategoryBySlug('striped-lungi'), price: 550, originalPrice: 699, images: ['https://picsum.photos/seed/product6/800/800', 'https://picsum.photos/seed/product6-2/800/800'], description: 'A modern take on the traditional lungi with stylish stripes. Perfect for lounging at home or a casual outing.', details: ['100% Cotton', 'Machine Washable', '2.25 meters length', 'Modern striped pattern'], stock: 90 },
  { id: 7, name: 'ADMK Flag Border Dhoti', slug: 'admk-flag-border-dhoti', category: getCategoryBySlug('political-party'), price: 950, originalPrice: 1299, images: ['https://picsum.photos/seed/product7/800/800'], description: 'Represent your party with pride. This pure cotton dhoti features the signature black, white, and red border, perfectly woven for political events and daily wear.', details: ['100% Premium Cotton', 'Colorfast Guarantee', '4 meters length', 'Authentic Party Colors'], stock: 5, virtualCategories: ['featured-products'] },
  { id: 8, name: 'Cotton Dhoti with Silk Border', slug: 'cotton-dhoti-silk-border', category: getCategoryBySlug('silk-border-dhoti'), price: 1100, originalPrice: 1400, images: ['https://picsum.photos/seed/product8/800/800'], description: 'The perfect blend of comfort and elegance. This soft cotton dhoti is enhanced with a shimmering silk border.', details: ['100% Cotton with Silk Border', 'Hand Wash Recommended', '4 meters length'], stock: 65 },
  { id: 9, name: 'Wedding Dhoti & Towel Set', slug: 'wedding-dhoti-towel-set', category: getCategoryBySlug('matching-dhoti'), price: 1899, originalPrice: 2499, images: ['https://picsum.photos/seed/product9/800/800'], description: 'A complete set for the groom, featuring a luxurious silk-blend dhoti and a matching towel, both with ornate zari borders.', details: ['Silk-Cotton Blend', 'Dry Clean Only', 'Perfect for wedding ceremonies'], stock: 30, virtualCategories: ['featured-products'] },
  { id: 10, name: 'Komarapalayam Hand Towel Set', slug: 'komarapalayam-hand-towel-set', category: getCategoryBySlug('towel'), price: 399, originalPrice: 599, images: ['https://picsum.photos/seed/product10/800/800'], description: 'A set of two soft and durable hand towels, woven in the traditional style of Komarapalayam. Perfect for daily use in the kitchen or bathroom.', details: ['100% Cotton', 'Machine Washable', 'Set of 2', 'Size: 40cm x 60cm'], stock: 200 },
];

export const attachReviewData = (products: Omit<Product, 'reviews' | 'rating' | 'reviewCount'>[], allReviews: Review[]): Product[] => {
...
export const baseProducts: Product[] = attachReviewData(baseProductsRaw, baseReviews);

export const baseUsers: User[] = [
    { id: 1, name: 'Suresh P.', email: 'suresh@example.com', password: btoa('password123'), phone: '9876543210', birthday: '1990-05-15', addresses: [{ id: 1, name: 'Suresh P. (Home)', street: "123, Weaver's Colony", city: 'Komarapalayam, Tamil Nadu', zip: '638183', isDefault: true }, { id: 2, name: 'Suresh P. (Work)', street: "456, Main Road", city: 'Erode, Tamil Nadu', zip: '638001' }] },
    { id: 2, name: 'Ramesh K.', email: 'ramesh@example.com', password: btoa('password123'), phone: '9876543211', addresses: [{ id: 3, name: 'Ramesh K. (Home)', street: "789, Gandhi Street", city: 'Chennai, Tamil Nadu', zip: '600001', isDefault: true }] },
    { id: 3, name: 'Gopal V.', email: 'gopal@example.com', password: btoa('password123'), phone: '9876543212', addresses: [{ id: 4, name: 'Gopal V. (Home)', street: "101, Bazaar Road", city: 'Madurai, Tamil Nadu', zip: '625001', isDefault: true }] },
    { id: 4, name: 'Anitha', email: 'anitha@example.com', password: btoa('password123'), phone: '9876543213', addresses: [{ id: 5, name: 'Anitha (Home)', street: "212, Kovai Nagar", city: 'Coimbatore, Tamil Nadu', zip: '641001', isDefault: true }] }
];

export const baseOrders: Order[] = [
    { id: 'LM-1024', userId: 1, date: '2023-10-15', total: 1398, status: 'Delivered', customerName: 'Suresh P.', paymentMethod: 'Prepaid', trackingProvider: 'Delhivery', trackingNumber: 'AWB987654321', items: [ { ...baseProducts.find(p => p.id === 1)!, quantity: 1 }, { ...baseProducts.find(p => p.id === 5)!, quantity: 1 } ], reviewedProducts: { 1: true, 5: false } },
    { id: 'LM-1023', userId: 2, date: '2023-10-01', total: 899, status: 'Delivered', customerName: 'Ramesh K.', paymentMethod: 'Prepaid', trackingProvider: 'Blue Dart', trackingNumber: 'BD123456789', items: [ { ...baseProducts.find(p => p.id === 2)!, quantity: 1 } ], reviewedProducts: { 2: true } },
    { id: 'LM-1022', userId: 3, date: '2023-09-29', total: 950, status: 'Shipped', customerName: 'Gopal V.', paymentMethod: 'Prepaid', trackingProvider: 'Ecom Express', trackingNumber: 'EE987123456', items: [ { ...baseProducts.find(p => p.id === 4)!, quantity: 1 } ], reviewedProducts: {} },
    { id: 'LM-1021', userId: 4, date: '2023-09-28', total: 550, status: 'Processing', customerName: 'Anitha', paymentMethod: 'Prepaid', items: [ { ...baseProducts.find(p => p.id === 6)!, quantity: 1 } ], reviewedProducts: { 6: false } },
];

export const baseCoupons: Coupon[] = [
...
export const getAvailableCouponsForProduct = (productId: number): Coupon[] => {
    return baseCoupons.filter(coupon => {
        if (!coupon.isActive) return false;
        if (coupon.trigger !== 'none') return false;
        if (coupon.applicableProductIds === undefined || coupon.applicableProductIds.length === 0) return true;
        return coupon.applicableProductIds.includes(productId);
    });
};

export const getProductsByCategory = (slug: string, products: Product[]): Product[] => {
