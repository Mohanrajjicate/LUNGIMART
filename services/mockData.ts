...
export const getAvailableCouponsForProduct = (productId: number): Coupon[] => {
    return coupons.filter(coupon => {
        ...
        if (coupon.trigger !== 'none') return false; // Only show general, non-triggered coupons on product page
        if (coupon.applicableProductIds === undefined || coupon.applicableProductIds.length === 0) return true; // Applies to all products
