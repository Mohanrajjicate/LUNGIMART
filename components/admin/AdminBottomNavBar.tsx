...
const AdminBottomNavBar: React.FC = () => {
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-200 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] grid grid-cols-4 items-stretch z-40 md:hidden">
            {navLinks.map(link => (
                 <NavLink key={link.to} to={link.to} end className={getNavLinkClass}>
                    {link.icon}
